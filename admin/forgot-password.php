...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'send_otp') {
        ...
        $stmt = $conn->prepare("SELECT id, name, email FROM users WHERE email = ? AND role IN ('admin', 'employee')");
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO password_reset_otp (email, otp, expiry, user_type) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL 10 MINUTE), 'admin')");
    ...
    if ($action === 'verify_otp') {
        ...
        $stmt = $conn->prepare("SELECT * FROM password_reset_otp WHERE email = ? AND otp = ? AND expiry > NOW() AND user_type = 'admin'");
        $stmt->bind_param("ss", $email, $otp);
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    if ($action === 'reset_password') {
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("UPDATE users SET password = ? WHERE email = ? AND role IN ('admin', 'employee')");
...
<head>
    ...
    <title>Forgot Password - JP Tiles Admin</title>
