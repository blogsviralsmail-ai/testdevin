...
interface Notice {
  ...
  center_id: number | null; center_name: string | null;
}
...
export default function CenterAnnouncements() {
  ...
  const myNotices = filtered.filter(n => n.center_id === centerId);
  const adminNotices = filtered.filter(n => !n.center_id);
  ...
  return (
    <div>
      ...
      <div className="mb-4 relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search announcements..." className="w-full pl-10 pr-4 py-2 border rounded-lg text-sm" />
      </div>

      {/* My Announcements */}
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-emerald-800 mb-3">My Announcements ({myNotices.length})</h2>
        <div className="space-y-3">
          {myNotices.map(n => (
            <div key={n.id} className={`bg-white rounded-xl border p-4 ${n.is_pinned ? "border-emerald-300 bg-emerald-50/30" : "border-gray-200"}`}>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    {n.is_pinned ? <Pin className="h-4 w-4 text-emerald-500" /> : null}
                    <h3 className="font-semibold text-gray-900">{n.title}</h3>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${priorityColor(n.priority)}`}>{n.priority}</span>
                    <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">{n.category}</span>
                ...
                <div className="flex gap-2 ml-4">
                  <button onClick={() => openEdit(n)} className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg"><Edit2 className="h-4 w-4" /></button>
                  <button onClick={() => del(n.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            </div>
          ))}
          {myNotices.length === 0 && <div className="text-center py-8 text-gray-500 bg-white rounded-xl border">No announcements created yet. Click "New Announcement" to create one for your students.</div>}
        </div>
      </div>

      {/* Admin Announcements (read-only) */}
      <div>
        <h2 className="text-lg font-semibold text-blue-800 mb-3">Admin Announcements ({adminNotices.length})</h2>
        <div className="space-y-3">
          {adminNotices.map(n => (
            <div key={n.id} className={`bg-white rounded-xl border p-4 ${n.is_pinned ? "border-blue-300 bg-blue-50/30" : "border-gray-200"}`}>
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-600">From Admin</span>
                </div>
                <p className="text-sm text-gray-600 mt-1 line-clamp-2">{n.content}</p>
                <p className="text-xs text-gray-400 mt-2">{new Date(n.created_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</p>
              </div>
          ...
          {adminNotices.length === 0 && <div className="text-center py-8 text-gray-500 bg-white rounded-xl border">No admin announcements</div>}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b">
              <h2 className="text-lg font-semibold">{editing ? "Edit Announcement" : "New Announcement"}</h2>
              <button onClick={() => setShowForm(false)}><X className="h-5 w-5 text-gray-500" /></button>
            </div>
            <div className="p-4 bg-emerald-50 border-b border-emerald-200 text-sm text-emerald-800">
              This announcement will be visible only to your center's students.
            </div>
            ...
            <div className="p-4 border-t flex gap-3 justify-end">
              ...
              <button onClick={save} className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700">
