...
CREATE TABLE IF NOT EXISTS live_streams (
    ...
    platform        ENUM('youtube','facebook') DEFAULT 'youtube',
    channel_id      INT DEFAULT NULL,
    stream_key      VARCHAR(500) DEFAULT '',
    title           VARCHAR(500) DEFAULT '',
    mode            ENUM('once','loop') DEFAULT 'once',
    status          ENUM('starting','live','stopping','stopped','error') DEFAULT 'starting',
...
CREATE TABLE IF NOT EXISTS live_videos (
    ...
    file_path       VARCHAR(500) DEFAULT '',
    file_size       BIGINT DEFAULT 0,
    quality         VARCHAR(10) DEFAULT '720',
    duration        VARCHAR(20) DEFAULT '',
    status          ENUM('uploaded','encoding','ready','failed') DEFAULT 'uploaded',
    created_at      DATETIME DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_user (user_id),
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB;
