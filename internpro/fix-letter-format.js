...
async function fixLetterFormats() {
  ...
  const LH = `<table style="width:100%;border-collapse:collapse;margin:0;padding:0;"><tr><td style="width:140px;vertical-align:middle;padding:8px 0 8px 28px;"><img src="${lhLogo}" alt="${esc(lhCompany)}" style="height:90px;display:block;object-fit:contain;" /></td><td style="text-align:right;vertical-align:middle;padding:8px 28px 8px 14px;"><p style="margin:0;font-size:22px;font-weight:700;color:#0000AA;letter-spacing:0.5px;">${esc(lhCompany)}</p><p style="margin:3px 0 0;font-size:13px;color:#555;line-height:1.3;">${esc(lhAddress)}</p><p style="margin:2px 0 0;font-size:13px;color:#555;">Ph: ${esc(lhPhone)} &nbsp;|&nbsp; ${esc(lhEmail)} &nbsp;|&nbsp; GST: ${esc(lhGst)}</p></td></tr></table><div style="height:3px;background:linear-gradient(90deg,#0000AA,#0000AA 70%,#d32f2f 70%,#d32f2f);"></div>`;
  const FT = `<div style="height:2px;background:linear-gradient(90deg,#0000AA,#0000AA 70%,#d32f2f 70%,#d32f2f);margin-top:auto;"></div><div style="padding:4px 28px;text-align:center;"><p style="margin:0;font-size:12px;font-weight:600;color:#0000AA;">${esc(lhCompany)}</p><p style="margin:2px 0 0;font-size:10px;color:#666;">${esc(lhAddress)} &nbsp;|&nbsp; Ph: ${esc(lhPhone)} &nbsp;|&nbsp; ${esc(lhEmail)} &nbsp;|&nbsp; GST: ${esc(lhGst)}</p></div>`;
...
<div style="background:#f5f5f5;padding:6px 14px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;"><span style="font-size:12px;color:#555;">Ref: <strong style="color:#222;">${esc(ol.letterNumber)}</strong></span><span style="font-size:12px;color:#555;">Date: <strong style="color:#222;">${todayDate}</strong></span></div>
...
<p style="font-size:13px;color:#333;margin:0 0 5px;">Dear <strong style="color:#0000AA;">${studentName}</strong>,</p>
<p style="font-size:12px;color:#333;line-height:1.4;text-align:justify;margin:0 0 8px;">With reference to your application and subsequent interactions, we are pleased to offer you an internship position at <strong>${esc(lhCompany)}</strong> for the <strong>${programName}</strong> program. This offer is subject to the following terms and conditions outlined below.</p>
...
<tr><td style="padding:4px 10px;border:1px solid #e0e0e0;font-weight:600;color:#333;background:#fafbff;">Program</td><td style="padding:4px 10px;border:1px solid #e0e0e0;">${programName}</td></tr>
...
<tr><td style="padding:4px 10px;border:1px solid #e0e0e0;font-weight:600;color:#333;">Work Timing</td><td style="padding:4px 10px;border:1px solid #e0e0e0;">${workTiming}</td></tr>
...
<tr><td style="padding:4px 10px;border:1px solid #e0e0e0;font-weight:600;color:#333;">Paid Leaves</td><td style="padding:4px 10px;border:1px solid #e0e0e0;">${paidLeaves} per month</td></tr>
...
<p style="font-size:12px;color:#333;line-height:1.35;text-align:justify;margin:0 0 6px;">We look forward to your valuable association with ${esc(lhCompany)} and wish you a highly productive and rewarding internship experience with us.</p>
<p style="margin:4px 0 0;font-size:12px;color:#333;">For &amp; on behalf of <strong style="color:#0000AA;">${esc(lhCompany)}</strong>,</p>
<div style="margin-top:3px;">
${sigBlock}
${signatoryName ? `<p style="margin:0;font-weight:700;color:#0000AA;font-size:15px;">${esc(signatoryName)}</p>` : ""}
<p style="margin:1px 0 0;font-size:12px;color:#555;">${esc(signatoryDesignation)}</p>
<p style="margin:1px 0 0;font-size:12px;color:#555;">${esc(lhCompany)}</p>
</div>
...
<p style="font-size:12px;color:#333;line-height:1.35;margin:0 0 4px;">I, <strong>${studentName}</strong>, hereby accept the above-mentioned terms and conditions and agree to abide by all policies, rules, and regulations of ${esc(lhCompany)} during the course of my internship.</p>
<table style="width:100%;font-size:12px;color:#555;"><tr><td style="width:50%;padding:3px 0;">Signature: ________________________</td><td style="width:50%;padding:3px 0;">Date: ________________________</td></tr><tr><td style="padding:3px 0;">Name: ${studentName}</td><td></td></tr></table>
</div>
</div>
<div style="flex-shrink:0;">${FT}</div>
</div>
...
${LH}
<div style="flex:1;padding:10px 36px 4px;">
<div style="background:#f5f5f5;padding:6px 14px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;"><span style="font-size:12px;color:#555;">Ref: <strong style="color:#222;">${esc(el.letterNumber)}</strong></span><span style="font-size:12px;color:#555;">Date: <strong style="color:#222;">${todayFormatted}</strong></span></div>
<div style="text-align:center;margin:0 0 10px;"><h2 style="margin:0;font-size:24px;font-weight:700;color:#0000AA;letter-spacing:3px;text-transform:uppercase;">Experience Certificate</h2><div style="width:40px;height:2px;background:#d32f2f;margin:4px auto 0;"></div></div>
<p style="font-size:13px;color:#333;margin:0 0 5px;"><strong>To Whom It May Concern,</strong></p>
<p style="font-size:12px;color:#333;line-height:1.4;text-align:justify;margin:0 0 8px;">This is to certify that <strong style="color:#0000AA;">${studentName}</strong> was associated with <strong>${esc(lhCompany)}</strong> as an intern under the <strong>${programTitle}</strong> program. The details of the engagement are summarized in the table below:</p>
<table style="width:100%;border-collapse:collapse;margin:0 0 8px;font-size:12px;border:1px solid #ddd;">
<tr style="background:#0000AA;"><td style="padding:4px 10px;color:white;font-weight:600;width:35%;border:1px solid #0000AA;">Particulars</td><td style="padding:4px 10px;color:white;font-weight:600;border:1px solid #0000AA;">Details</td></tr>
<tr><td style="padding:4px 10px;border:1px solid #e0e0e0;font-weight:600;color:#333;background:#fafbff;">Program</td><td style="padding:4px 10px;border:1px solid #e0e0e0;">${programTitle}</td></tr>
<tr><td style="padding:4px 10px;border:1px solid #e0e0e0;font-weight:600;color:#333;">Duration</td><td style="padding:4px 10px;border:1px solid #e0e0e0;">${e.batch.program.duration} Days</td></tr>
<tr><td style="padding:4px 10px;border:1px solid #e0e0e0;font-weight:600;color:#333;background:#fafbff;">Period</td><td style="padding:4px 10px;border:1px solid #e0e0e0;">${startDateStr} to ${endDateStr}</td></tr>
<tr><td style="padding:4px 10px;border:1px solid #e0e0e0;font-weight:600;color:#333;">Performance</td><td style="padding:4px 10px;border:1px solid #e0e0e0;"><strong style="color:#0000AA;">${performanceLabel}</strong></td></tr>
</table>
<p style="font-size:13px;font-weight:700;color:#0000AA;margin:8px 0 4px;text-decoration:underline;">Performance Summary</p>
<p style="font-size:12px;color:#333;line-height:1.35;text-align:justify;margin:0 0 6px;">During the internship tenure, ${studentName} demonstrated commendable professionalism, technical aptitude, and a consistently proactive approach towards learning and skill development. The intern met assigned deadlines with diligence, exhibited strong analytical and problem-solving capabilities, and collaborated effectively with team members across various projects. The overall quality of work delivered was rated as <strong style="color:#0000AA;">${performanceLabel}</strong> by the supervising authority.</p>
<p style="font-size:13px;font-weight:700;color:#0000AA;margin:8px 0 4px;text-decoration:underline;">Key Strengths Observed</p>
<ul style="font-size:12px;color:#333;line-height:1.35;margin:0 0 6px;padding-left:20px;">
<li style="margin-bottom:2px;">Strong understanding of core concepts and practical applications related to the ${programTitle} domain.</li>
...
<p style="font-size:13px;font-weight:700;color:#0000AA;margin:8px 0 4px;text-decoration:underline;">Recommendation</p>
...
<p style="margin:8px 0 0;font-size:12px;color:#333;">For &amp; on behalf of <strong style="color:#0000AA;">${esc(lhCompany)}</strong>,</p>
<div style="margin-top:3px;">
${sigBlockExp}
${signatoryName ? `<p style="margin:0;font-weight:700;color:#0000AA;font-size:15px;">${esc(signatoryName)}</p>` : ""}
<p style="margin:1px 0 0;font-size:12px;color:#555;">${esc(signatoryDesignation)}</p>
