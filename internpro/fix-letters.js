...
async function main() {
  ...
  const LH = `<table style="width:100%;border-collapse:collapse;margin:0;padding:0;"><tr><td style="width:180px;vertical-align:middle;padding:12px 0 12px 28px;"><img src="${lhLogo}" alt="${cn}" style="height:164px;display:block;object-fit:contain;" /></td><td style="text-align:right;vertical-align:middle;padding:12px 28px 12px 14px;"><p style="margin:0;font-size:28px;font-weight:700;color:#0000AA;letter-spacing:0.5px;">${cn}</p><p style="margin:5px 0 0;font-size:16px;color:#555;line-height:1.4;">${esc(lhAddress)}</p><p style="margin:4px 0 0;font-size:16px;color:#555;">Ph: ${esc(lhPhone)} &nbsp;|&nbsp; ${esc(lhEmail)} &nbsp;|&nbsp; GST: ${esc(lhGst)}</p></td></tr></table><div style="height:4px;background:linear-gradient(90deg,#0000AA,#0000AA 70%,#d32f2f 70%,#d32f2f);"></div>`;

  const FT = `<div style="height:3px;background:linear-gradient(90deg,#0000AA,#0000AA 70%,#d32f2f 70%,#d32f2f);margin-top:auto;"></div><div style="padding:8px 28px;text-align:center;"><p style="margin:0;font-size:14px;font-weight:600;color:#0000AA;">${cn}</p><p style="margin:3px 0 0;font-size:12px;color:#666;">${esc(lhAddress)} &nbsp;|&nbsp; Ph: ${esc(lhPhone)} &nbsp;|&nbsp; ${esc(lhEmail)} &nbsp;|&nbsp; GST: ${esc(lhGst)}</p></div>`;
  ...
  const sigNameLine = sigName ? `<p style="margin:0;font-weight:700;color:#0000AA;font-size:18px;">${esc(sigName)}</p>` : "";
  ...
  const extraSection = extraTerms ? `<p style="font-size:14px;font-weight:700;color:#0000AA;margin:5px 0 2px;">9. Additional Terms</p><p style="font-size:13px;color:#333;line-height:1.45;text-align:justify;margin:0 0 3px;">${esc(extraTerms)}</p>` : "";
  const extraExpSec = extraExpNote ? `<p style="font-size:16px;color:#333;line-height:1.55;text-align:justify;margin:4px 0 8px;">${esc(extraExpNote)}</p>` : "";
...
${LH}
<div style="${PC}">
<table style="width:100%;margin-bottom:4px;"><tr><td style="font-size:13px;color:#555;">Ref: <strong style="color:#222;">${letterNumber}</strong></td><td style="text-align:right;font-size:13px;color:#555;">Date: <strong style="color:#222;">${todayDate}</strong></td></tr></table>
...
<p style="font-size:13px;color:#333;line-height:1.45;text-align:justify;margin:0 0 3px;">With reference to your application and subsequent interactions, we are pleased to offer you an internship position at <strong>${cn}</strong> for the <strong>${programName}</strong> program. This offer is subject to the following terms and conditions outlined below.</p>
...
<tr><td style="padding:5px 12px;border:1px solid #e0e0e0;font-weight:600;color:#333;background:#fafbff;">Date of Joining</td><td style="padding:5px 12px;border:1px solid #e0e0e0;">${joiningDate}</td></tr>
...
<p style="font-size:13px;color:#333;line-height:1.45;text-align:justify;margin:0 0 3px;">We look forward to your valuable association with ${cn} and wish you a highly productive and rewarding internship experience with us.</p>
<p style="margin:5px 0 0;font-size:14px;color:#333;">For &amp; on behalf of <strong style="color:#0000AA;">${cn}</strong>,</p>
<div style="margin-top:4px;">
${sigBlock}
${sigNameLine}
<p style="margin:2px 0 0;font-size:13px;color:#555;">${esc(sigDesg)}</p>
<p style="margin:2px 0 0;font-size:13px;color:#555;">${cn}</p>
</div>
...
<p style="font-size:13px;color:#333;line-height:1.45;margin:0 0 3px;">I, <strong>${studentName}</strong>, hereby accept the above-mentioned terms and conditions and agree to abide by all policies, rules, and regulations of ${cn} during the course of my internship.</p>
...
<div style="flex-shrink:0;">${FT}</div>
    ...
    console.log(`  Updated offer letter: ${letterNumber} (${e.student.name})`);
  }
  ...
  for (const el of expLetters) {
    ...
    const remarks = e.teamLeaderRemarks ? esc(e.teamLeaderRemarks) : "";
    ...
    const endDateStr = e.completedAt ? new Date(e.completedAt).toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" }) : new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" });
    ...
    const expSigName = sigName ? `<p style="margin:0;font-weight:700;color:#0000AA;font-size:18px;">${esc(sigName)}</p>` : "";
...
<p style="font-size:13px;color:#333;line-height:1.45;text-align:justify;margin:0 0 3px;">This is to certify that <strong style="color:#0000AA;">${studentName}</strong> was associated with <strong>${cn}</strong> as an intern under the <strong>${programName}</strong> program. The details of the engagement are summarized in the table below:</p>
...
${remarks ? `<tr><td style="padding:5px 12px;border:1px solid #e0e0e0;font-weight:600;color:#333;background:#fafbff;">Remarks</td><td style="padding:5px 12px;border:1px solid #e0e0e0;">${remarks}</td></tr>` : ""}
</table>
...
<ul style="font-size:13px;color:#333;line-height:1.45;margin:0;padding-left:20px;">
<li style="margin-bottom:2px;">Strong understanding of core concepts and practical applications related to the ${programName} domain.</li>
...
${extraExpSec}
<p style="margin:10px 0 0;font-size:14px;color:#333;">For &amp; on behalf of <strong style="color:#0000AA;">${cn}</strong>,</p>
