      ...
      <div className="rounded-xl p-4 mb-8" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)" }}>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <select
              value={state}
              onChange={(e) => { handleFilterChange(setState, e.target.value); setCity(""); }}
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <select
              value={sortBy}
              onChange={(e) => handleFilterChange(setSortBy as (v: string) => void, e.target.value)}
              className="px-3 py-2 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/50"
              style={selectStyle}
            >
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => handleFilterChange(setCategory, "")}
          ...
          {categories.map((c) => (
            <button key={c} onClick={() => handleFilterChange(setCategory, c)}
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3 pt-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            {state && (
              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-purple-500/20 text-purple-300">
                {state}
                <button onClick={() => { handleFilterChange(setState, ""); setCity(""); }} className="hover:text-white">&times;</button>
              </span>
            )}
            {city && (
              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-emerald-500/20 text-emerald-300">
                {city}
                <button onClick={() => handleFilterChange(setCity, "")} className="hover:text-white">&times;</button>
              </span>
            )}
            {category && (
              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-cyan-500/20 text-cyan-300">
                {category}
                <button onClick={() => handleFilterChange(setCategory, "")} className="hover:text-white">&times;</button>
              </span>
            )}
            {search && (
              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-amber-500/20 text-amber-300">
                &quot;{search}&quot;
                <button onClick={() => handleFilterChange(setSearch, "")} className="hover:text-white">&times;</button>
              </span>
            )}
            ...
            <Link key={blog.id} href={`/blog/${blog.slug}`}
              ...
              {blog.coverImage ? (
                <div className="aspect-video overflow-hidden">
                  <img src={blog.coverImage} alt={blog.title} width={400} height={210} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
                </div>
              ) : (
              ...
              <div className="p-5">
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <span className="text-xs text-cyan-400 group-hover:translate-x-1 transition-transform">Read More &rarr;</span>
                </div>
              </div>
            </Link>
          ))}
