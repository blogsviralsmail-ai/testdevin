...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existing) return NextResponse.json({ message: "Offer letter already exists", letterNumber: existing.letterNumber });
...
<tr><td style="padding:5px 12px;border:1px solid #e0e0e0;font-weight:600;background:#fafbff;">Fee Type</td><td style="padding:5px 12px;border:1px solid #e0e0e0;">Paid — ₹${(enrollment.feeAmount || 0).toLocaleString()}</td></tr>
...
${FT}
    ...
    logActivity("generated", "offer_letter", offerLetter.id, `Offer Letter ${offerLetter.letterNumber} for ${enrollment.student.name} (after payment)`, session.id, session.name).catch(() => {});

    return NextResponse.json({ success: true, letterNumber: offerLetter.letterNumber });
  } catch (error: unknown) {
