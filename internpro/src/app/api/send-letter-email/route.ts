...
export async function POST(request: NextRequest) {
  ...
  // Build a proper email body (not the raw letter HTML)
  const emailBody = `<h2 style="color:#1f2937;margin:0 0 12px;">Dear ${studentName},</h2>
<p style="color:#4b5563;line-height:1.7;margin:0 0 10px;">Your <strong>${letterType}</strong> has been generated by <strong>KKHS Media Private Limited</strong>. Please find the document attached as a PDF with this email.</p>
...
<p style="color:#9ca3af;font-size:12px;margin-top:20px;">This is an automated email from KKHS Media Internship Platform.</p>`;
  ...
  const sent = await sendEmail({ to: email, subject: `${letterType} — KKHS Media`, html: emailBody, attachments });
  if (sent) {
