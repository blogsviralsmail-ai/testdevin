...
<p style="font-size:16px;color:#333;line-height:1.85;text-align:justify;margin:0 0 14px;">This is to certify that <strong style="color:#0000AA;">${safeStudentName}</strong> was associated with <strong>${escapeHtml(lhCompany)}</strong> as an intern under the <strong>${safeProgramTitle}</strong> program. The details of the engagement are summarized in the table below:</p>
...
${extraExpSection}
<p style="margin:22px 0 0;font-size:17px;color:#333;">For &amp; on behalf of <strong style="color:#0000AA;">${escapeHtml(lhCompany)}</strong>,</p>
<div style="margin-top:12px;">
${sigBlock}
${signatoryName ? `<p style="margin:0;font-weight:700;color:#0000AA;font-size:18px;">${escapeHtml(signatoryName)}</p>` : ""}
<p style="margin:4px 0 0;font-size:15px;color:#555;">${escapeHtml(signatoryDesignation)}</p>
<p style="margin:4px 0 0;font-size:15px;color:#555;">${escapeHtml(lhCompany)}</p>
</div>
</div>
