...
export async function GET(request: NextRequest) {
  ...
  const enrollments = await prisma.enrollment.findMany({
    ...
    include: {
      student: {
        select: { id: true, name: true, email: true, phone: true, avatar: true, collegeName: true, degree: true, year: true, address: true, dob: true, employeeId: true, referredBy: { include: { agent: { include: { user: { select: { name: true, email: true } } } } } } },
      },
      batch: {
        include: {
