...
export default function TermsPage() {
  ...
  useEffect(() => {
    fetch("/api/site-content?slug=terms-and-conditions")
      .then(r => r.json())
      ...
      <div className="pt-28 pb-20 px-6">
        <div className="max-w-4xl mx-auto">
          <div className="mb-12 text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm mb-6" style={{ background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.2)', color: '#c4b5fd' }}>
              📋 Legal
            </div>
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              <span style={{ background: 'linear-gradient(135deg, #a78bfa, #22d3ee)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Terms & Conditions</span>
            </h1>
            ...
            {data.sections.map((s, i) => (
              <div key={i} className="p-6 md:p-8 rounded-2xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-3">
                  <span className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold" style={{ background: 'linear-gradient(135deg, #a78bfa, #0EA5B8)' }}>{i + 1}</span>
                  {s.title}
                </h2>
      ...
      <footer className="py-12 px-6" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6 text-sm text-slate-500">
            ...
            <Link href="/terms" className="hover:text-white transition text-white">Terms & Conditions</Link>
          </div>
