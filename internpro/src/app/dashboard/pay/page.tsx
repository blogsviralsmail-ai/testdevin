  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/student-payment", {
        ...
        body: JSON.stringify({ enrollmentId, paymentMethod, transactionId }),
      });
        ...
        <div className="p-6">
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[{v: "upi", l: "UPI"}, {v: "bank", l: "Bank Transfer"}, {v: "other", l: "Other"}].map(m => (
                <button key={m.v} onClick={() => setPaymentMethod(m.v)}
                  className={`p-3 rounded-lg border text-sm font-medium transition-all ${paymentMethod === m.v ? 'border-indigo-500 bg-indigo-50 text-indigo-700 ring-2 ring-indigo-200' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}>
                  {m.l}
                </button>
              ))}
            </div>
          ...
          <button onClick={handlePayment} disabled={processing}
            ...
            ) : `Pay ₹${enrollment.feeAmount.toLocaleString()} & Get Offer Letter`}
          </button>
        </div>
      </div>
    </div>
