    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl p-6 text-white">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <button onClick={copyCode} className="surface-base/20 hover:surface-base/30 px-3 py-1.5 rounded-lg text-sm">Copy Code</button>
          <button onClick={copyLink} className="surface-base/20 hover:surface-base/30 px-3 py-1.5 rounded-lg text-sm">Copy Link</button>
        </div>
        <div className="grid grid-cols-3 gap-4 mt-4">
          <div className="surface-base/10 rounded-lg p-3">
      ...
      <div className="surface-base rounded-lg border">
        ...
        {referrals.length === 0 ? (
          <div className="p-8 text-center text-tertiary">No referrals yet. Share your code to earn!</div>
        ) : (
            ...
            {referrals.map((ref: any, i: number) => (
              <div key={i} className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-tertiary">Joined: {new Date(ref.joinedAt).toLocaleDateString()}</p>
                </div>
                <span className={`px-2 py-1 rounded text-xs font-medium ${ref.status === 'active' ? 'bg-green-100 text-green-800' : 'surface-muted text-secondary'}`}>
                  {ref.status || 'active'}
                </span>
              </div>
            ))}
            ...
            {earnings.map((earn: any, i: number) => (
              <div key={i} className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-tertiary">From: {earn.referredUserName || 'User'} | Order: ₹{earn.orderAmount}</p>
                </div>
