  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={loadSlots} className="px-3 py-2 rounded-lg border hover:bg-[rgb(var(--bg-muted))]"><RefreshCw size={18} /></button>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b surface-subtle">
                <th className="text-left px-4 py-3 text-sm font-medium text-tertiary">Slot</th>
                <th className="text-left px-4 py-3 text-sm font-medium text-tertiary hidden md:table-cell">User</th>
                <th className="text-left px-4 py-3 text-sm font-medium text-tertiary">Platform</th>
                <th className="text-left px-4 py-3 text-sm font-medium text-tertiary">Status</th>
                <th className="text-left px-4 py-3 text-sm font-medium text-tertiary hidden lg:table-cell">Stream</th>
                <th className="text-right px-4 py-3 text-sm font-medium text-tertiary">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(slot => (
                <tr key={slot.id} className="border-b last:border-b-0 hover:bg-[rgb(var(--bg-muted))]">
                  <td className="px-4 py-3">
                    <div className="text-sm font-medium">{slot.name}</div>
                    <div className="text-xs text-tertiary">{slot.videoName || 'No video'}</div>
                  </td>
                  <td className="px-4 py-3 text-sm text-tertiary hidden md:table-cell">{slot.userEmail || slot.userId}</td>
                  <td className="px-4 py-3 text-sm capitalize">{slot.platform}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      slot.status === 'active' ? 'bg-green-100 text-green-700' : slot.status === 'expired' ? 'bg-red-100 text-red-700' : 'surface-muted text-secondary'
                    }`}>{slot.status}</span>
                  </td>
