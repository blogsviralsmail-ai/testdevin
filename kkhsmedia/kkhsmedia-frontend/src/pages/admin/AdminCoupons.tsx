  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-primary">Coupons & Discounts</h1>
      ...
      <div className="surface-base rounded-lg border">
        {coupons.length === 0 ? (
          <div className="p-8 text-center text-tertiary">No coupons yet</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="surface-subtle">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-tertiary">Code</th>
                  <th className="px-4 py-3 text-left font-medium text-tertiary">Discount</th>
                  <th className="px-4 py-3 text-center font-medium text-tertiary">Uses</th>
              ...
              <tbody className="divide-y">
                {coupons.map((c: any) => (
                  <tr key={c.id} className="hover:bg-[rgb(var(--bg-muted))]">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => toggleActive(c)} className={`px-2 py-1 rounded-full text-xs font-medium ${c.isActive ? 'bg-green-100 text-green-800' : 'surface-muted text-secondary'}`}>
