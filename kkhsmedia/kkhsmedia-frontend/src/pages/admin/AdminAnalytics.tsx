...
export default function AdminAnalytics() {
  ...
  if (loading) return <div className="text-center py-12 text-tertiary">Loading analytics...</div>;
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Total Users', value: ((analytics.totalUsers as number) || 0).toString(), icon: Users, color: '#6366f1' },
          ...
          <div key={i} className="surface-base rounded-xl border p-5">
            ...
            <div className="text-sm text-tertiary">{card.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        <div className="surface-base rounded-xl border p-6">
          ...
          {revenueByMonth.length === 0 ? (
            <p className="text-sm text-tertiary py-8 text-center">No revenue data yet</p>
          ) : (
                ...
                return (
                  <div key={i}>
                    ...
                    <div className="w-full surface-muted rounded-full h-2">
                      <div className="h-2 rounded-full" style={{ width: `${width}%`, backgroundColor: 'rgb(var(--accent))' }} />
                    </div>
                  </div>
                );
              })}
        ...
        <div className="surface-base rounded-xl border p-6">
          ...
          {usersByMonth.length === 0 ? (
            <p className="text-sm text-tertiary py-8 text-center">No user data yet</p>
          ) : (
            <div className="space-y-3">
      ...
      <div className="surface-base rounded-xl border p-6">
        ...
        {topPlans.length === 0 ? (
          <p className="text-sm text-tertiary py-8 text-center">No plan data yet</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b">
                  <th className="text-left py-2 text-sm font-medium text-tertiary">Plan</th>
                  <th className="text-left py-2 text-sm font-medium text-tertiary">Orders</th>
                  <th className="text-left py-2 text-sm font-medium text-tertiary">Revenue</th>
                </tr>
              </thead>
              <tbody>
                {topPlans.map((plan, i) => (
                  <tr key={i} className="border-b last:border-b-0">
                    ...
                    <td className="py-2 text-sm text-secondary">{plan.count}</td>
                    <td className="py-2 text-sm font-medium">₹{plan.revenue.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
