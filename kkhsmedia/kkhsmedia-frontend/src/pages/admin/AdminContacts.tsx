  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={loadContacts} className="px-3 py-2 rounded-lg border hover:bg-[rgb(var(--bg-muted))]"><RefreshCw size={18} /></button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 surface-base rounded-xl border">
          <MessageSquare size={40} className="mx-auto mb-3 text-secondary" />
          <p className="text-tertiary">No messages yet</p>
        </div>
      ) : (
        <div className="space-y-3">
          {contacts.map(contact => (
            <div key={contact.id} className="surface-base rounded-xl border p-5">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <a href={`mailto:${contact.email}`} className="text-sm text-tertiary flex items-center gap-1">
                      <Mail size={12} /> {contact.email}
                    </a>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-tertiary">{new Date(contact.createdAt).toLocaleDateString()}</span>
                  <button onClick={() => handleDelete(contact.id)} className="p-1.5 rounded hover:bg-red-50">
                    <Trash2 size={14} className="text-red-500" />
                  </button>
                </div>
              </div>
              <p className="mt-3 text-sm text-secondary surface-subtle rounded-lg p-3">{contact.message}</p>
            </div>
          ))}
        </div>
      )}
