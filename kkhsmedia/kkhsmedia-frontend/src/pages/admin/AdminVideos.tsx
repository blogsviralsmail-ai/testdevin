        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setPlayingVideo(null)}>
          <div className="bg-white rounded-2xl max-w-3xl w-full overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <button onClick={() => setPlayingVideo(null)} className="p-1 rounded-lg hover:bg-gray-100"><X size={20} /></button>
            </div>
            <div className="bg-black">
              <video controls autoPlay className="w-full max-h-[70vh]" src={getStreamUrl(playingVideo)} />
            </div>
            <div className="p-3 text-sm text-gray-500 flex justify-between">
          ...
          {filtered.map(video => {
            ...
            return (
              <div key={video.id} className="bg-white rounded-xl border overflow-hidden hover:shadow-md transition-shadow">
                {/* Thumbnail */}
                <div className="relative aspect-video bg-gray-100 cursor-pointer group" onClick={() => setPlayingVideo(video)}>
                  {thumbUrl ? (
                ...
                <div className="p-3">
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-xs text-gray-500">{video.userEmail || video.userId}</span>
